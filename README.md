...
# 大規模言語モデル評価フレームワーク swallow-evaluation-instruct Ver. 202604
...
MODEL_ARGS には `model` や `base_url` を指定します．modelパラメータは `{プロバイダ名}/{MODEL ID}` という表記にします（例："openai/gpt-5.2-2025-12-11"）．
...
[vLLM serveコマンド](https://docs.vllm.ai/en/stable/serving/openai_compatible_server.html)でOpenAI互換APIを起動し，そのAPIをLiteLLM経由で呼び出すことにより，[推論型モデルサポート](https://docs.vllm.ai/en/stable/features/reasoning_outputs.html)などのvLLMの豊富な機能を活用しながら評価を実行できます．

以下の手順のいずれかで vllm を起動する際に torch から `InductorError` が発生した場合
（例: `SystemError: PY_SSIZE_T_CLEAN macro must be defined for '#' formats`）は、
次のコマンドを実行してください。
...
```
@misc{swallow-evaluation-instruct-v202604,
